...
**修復進度（2026-08-14）：✅ 已完成**。#12A 已讓搜尋、排課與 AI Agent 共用分類流程，完成必修、核心選修、一般選修與系外選修；#12B 已補上通識正式分類、領域及課程開設學年度規則。

**結論**：MySQL `type` 仍只提供必修／選修，但分類層已補出資工核心選修、一般選修、系外選修與通識。通識依官方 `dept` 領域、課程學年度及跨院認抵表處理，不使用課號前綴猜測。
...
- `server/src/data/generalEducationCatalog.js`：111 以前、112～114、115 起三個通識分類版本及 114-2 跨院認抵課對照。
- `server/src/skills/courseQuery.js`：先分類再搜尋，明確 `category=通識` 可跨本人班級範圍。
- `server/src/data/generalEducation.js`：保留處理不計畢業學分課程，與通識選修分類分工。

**資料庫證據**：`Courses.type`＝必修 1,760、選修 1,326；114-2 四個官方通識 `dept` 共 167 個正式課號、208 個班次且零缺課號；三門官方跨院認抵課均可唯一對回正式課號。

**正式執行資料來源**：MySQL 原始類別、資工正式科目表、通識中心學年度規則與跨院認抵表。

**剩餘邊界**：舊學期完整畢業認列需要歷史課程檢索與學生入學年度，已移至 #23；不影響目前 114-2 課程搜尋與排課分類。
...
|---|---|---|---|---|---|
| #12 | ✅ 已完成 | 五類已統一；通識含學年度、領域與 114-2 認抵課 | 已查 | 搜尋/排課 | — |
