...
建模時間 1～3.5 ms，LP 文字約 25 KB。單次呼叫前後的 RSS 差值：第一次約 37 MB（WASM heap 擴張），之後都在 10 MB 以內。這只是單次呼叫前後的差值觀測，不代表長時間執行不會累積（2026-09-19 補充 500 次連續求解的趨勢觀測，見 [MILP 任務 1 報告](./2026-09-19-roadmap-10-milp-candidate-generation.md)）。
...
- 因此，正式模型把「學分不少於 S₀（或使用者指定的目標學分）」列為硬限制，讓替代方案與 S₀ 在相同學分下公平比較。（2026-09-19 修正用語：原寫「在上限內排滿」，但 10～11 學分其實是目前目標函數在下限 9 學分下的最優解，不是求解器提早停止。）
