...
### 修正三：`SetupPage` 儲存時會清掉已存的班別

該頁的 `className` 初始值取自登入回傳的 `user` 物件，其中沒有 `className`，
因此下拉永遠顯示「未指定班別」。後果是**進入設定頁、沒有碰班別就按儲存，
已存的班別會被清成空值**——屬本次新增功能引入的資料遺失缺陷。

修法：改由 `GET /api/profile` 載入系所、年級與班別，並在 profile 載入完成前
停用送出按鈕（顯示「載入設定中...」），避免用表單的暫時預設值覆蓋已儲存的設定。

### 修正四：F16（詳見 `2026-08-01-frontend-backend-alignment-audit.md`）

由使用者實測發現：直接手改 `server/data/users.json` 的班級與年級後排課不變。

- **F16-a**：`users.json` 的 `department`／`grade` 是死欄位——真相來源是
  `user_preferences`，但 `graduation.js` 又讀 `users.json`。已從 `users.json` 移除這兩個
  欄位，`graduation.js` 改讀同一份 profile。順帶修掉該檔用班級名稱比對系所全名
  （`資訊三甲` vs `資訊工程學系`）而永遠不成立的既有缺陷。
- **F16-b**：班別與年級不一致時原本「忽略班別」，導致改了班別課表毫無變化。
  改為**年級以班別為準**（班別名稱本身編碼了年級），並回報兩邊的年級不一致。
  只有班別的**系所**對不上時才忽略班別。

## 第四輪：把靜默失敗變成明確錯誤，並補上資料庫契約測試

### 明確指定豁免（擴大到必修範圍判定）

先前只有系外選修的認列條件會豁免使用者明確指定的課程；他班／他系的必修仍被整批排除。
但那個判定是「依系所、年級、班別**推論**」，不是校方的選課權限——轉系、輔系、雙主修、
跨班加簽都可能讓學生真的修得到。

改為：系統自撿的候選照舊排除，**使用者明確指定的保留並排入**，並警告
「系上不接受必修換班，能否修習需自行向系辦確認」。

### 對照失敗報錯

`buildStudentScope()` 新增 `departmentMissing` / `departmentUnmapped` / `gradeMissing`。
先前「沒填系所」與「填了但對不到 A 表」都回報同一句「未設定系所或年級」，
後者是資料錯誤（系所名稱打錯或 A 表缺漏）卻被當成使用者還沒填，永遠查不出來。

現在：缺欄位 → 警告並指出缺哪一項；對不到 A 表 → **回報為失敗**並指名系所。
`database.js` 讀取 profile 時另以 `logger.error` 記錄對不到 A 表的 `User_Profiles.department`。

### 未設定 MySQL 報錯

`courses` 與 `reviews` 只存在於 MySQL（種子資料已移除）。未設定 `DB_*` 時先前靜默回傳
空陣列，排課因此回報「找不到符合條件的候選課程」——看起來像篩選太嚴，實際上是資料庫
沒接上。改為丟出明確錯誤並指出要設哪三個環境變數。`user_preferences` 不在此限。

### 新增 `server/test/database-contract.test.js`

連真實 MySQL，斷言程式依賴的每一個資料假設（D1-D10，見 `docs/TEST_PLAN.md`）。
基準值取自 2026-08-04 實測，比例容忍 5%、數量容忍 25%。

未設定 `DB_*` 時整組標記為 **skip**（輸出顯示 `skipped 10`），不是靜默通過。
連線池會在 `after()` 關閉，否則 test runner 的 event loop 不會結束、`npm test` 永遠不返回。

**驗證**：連 DB 執行 10 pass / 0 fail；移除 `.env` 後 10 skipped / 0 pass。
全套 `npm test` 225 pass / 0 fail。
