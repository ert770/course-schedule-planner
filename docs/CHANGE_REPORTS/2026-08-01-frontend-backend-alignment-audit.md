...
2026-08-04（新增並完成 F16）／2026-08-01（完成 F1、F2、F3、F14；新增 F15）
...
| 改動 | 內容 |
| --- | --- |
| `server/data/users.json` | 移除 `department` 與 `grade`。該檔只留登入身分與 demo 展示資料，不再是系所／年級的來源 |
| `server/src/routes/graduation.js` | 系所改讀 `user_preferences`（與排課同一份 profile）。先前讀 `users.json`，畢業進度與課表可依不同系所計算而毫無跡象 |
| `client/src/pages/SetupPage.jsx` | 系所、年級、班別改由 `GET /api/profile` 載入，不再用登入回傳的 `user` 物件當預設值 |

順帶修掉同一段的既有缺陷：`graduation.js` 用 `course.department === user.department`
比對系上課程，等於拿班級名稱「資訊三甲」比系所全名「資訊工程學系」，**永遠不成立**，
這條建議從來沒出現過。改為與排課一致，先 `parseClassName()` 解析班級名稱再比對系所。

**F16-b　年級改以班別為準。**

`buildStudentScope()` 的判定改為：

| 情況 | 處理 |
| --- | --- |
| 班別是**本系**的系所班級 | **年級改依班別**（`資訊二乙` → 2 年級），並回報與個人資料不一致 |
| 班別的系所與 profile 不符（`電機三甲` 對資工） | 無從調解，忽略班別並警告，年級沿用 profile |
| 未設定班別 | 沿用系所 + 年級判定 |

理由：班別名稱本身就編碼了年級，且它是使用者最後明確選的值。原本「不一致就忽略班別」
的處理，正是使用者實測「改了班別課表卻毫無變化」的直接原因。

新增 scope 欄位 `gradeOverriddenByClass` 與 `profileGrade`，警告訊息會寫出兩邊的年級，
不讓資料不一致被靜默吞掉。

### 驗證（2026-08-04）

`npm test` 212 pass / 0 fail（新增 F16-b 的單元與端到端案例）。實機以 headless Chrome 驗證：

| 情境 | 結果 |
| --- | --- |
| 設定頁載入 | 班別下拉正確帶回 `資訊三甲`（先前顯示「未指定班別」） |
| 設定頁直接按儲存 | 班別仍為 `資訊三甲`（先前會被清空） |
| 只改班別為 `資訊二乙` | 課表換成二年級必修（系統程式、資料庫系統、資料結構、機率與統計），警告「班別「資訊二乙」為 2 年級，與個人資料的 3 年級不一致；已依班別判定」 |
| 改回 `資訊三甲` | 課表換回三年級必修 |
