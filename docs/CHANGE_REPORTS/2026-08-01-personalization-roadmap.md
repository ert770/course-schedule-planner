...
| 13C | B～F 類的正式適用規則 | ✅ **已完成（2026-09-18）**——依**專案負責人口頭確認**的規則實作（`classKindCatalog.js` 的 `ELIGIBILITY_RULES`，`eligibilitySource: class-catalog:owner-confirmed-2026-09-18`），**不是校方書面文件**；B 依年級、C 依學院、D／F 不可修、E 不需報名；同系他年級選修可修但排序在本年級之後。仍回 `unknown`：`進修英班`／`大二進修英班`、研究生對碩士綜合班。見下方 #13C 段落與[變更報告](./2026-09-18-implement-13c-13d-eligibility-rules.md) | #13B（已完成）；規則來源為本人口頭確認，日後若取得校方文件需回頭核對 |
| 13D | 學制、學程與特殊身分 | ✅ **已完成（2026-09-18）**——學分學程不需報名，原規劃的「已報名學程」欄位不再需要；D 類獨立學制不在 `User_Profiles.department` 的值域內，一律不可修；碩士綜合班大學部不可修；`User_Profiles.department` 的值域已確認（A 表系所全名或學士綜合班名稱，大三起改為分流後系所）。見下方 #13D 段落 | #13B、#18（均已完成） |
...
| 20 | 建立 active term 與完整 candidate eligibility 規則 | ✅ **已完成（2026-09-18）**——原本剩下的兩個缺口（B～F 適用規則、學制學程身分）隨 #13C／#13D 完成；沒有規則的班級（進修英班）與研究生對碩士綜合班仍明確回 `unknown`，那是刻意的邊界，不是遺漏 | #12、#13A、#13B、#13C、#13D、#18、#19（均已完成） |
...
| 23 | 建立版本化且可追溯的畢業規則引擎 | 🟡 部分完成（2026-09-21）——版本化架構、逐門認列追溯、規則來源記錄、補學分推薦、通識拆分，及「歷史缺口 ÷ 剩餘學期」接入正式排課皆完成；本系選修最多 3 門，達配額後改排 1+1／2 通識／2 系外，替代 MILP 維持 S₀ 類別門數。仍缺的規則全卡在外部資料。**2026-09-18 新增缺口**仍存在：B～F 可修課程算哪一類畢業學分沒有正式規則 | #12、#19（均已完成）；另需校方正式規則 |
...
`#10` 的任務 1 已於 2026-09-19 接上正式 MILP 候選池；最新五個 case 有三個通過、兩個因
訊號或主軸可行性不足而失敗。任務 2～4 尚未進行，因此仍列在可動工項目。
...
- **卡外部資料，不是排程問題**：`#6`／`#32`（足夠真實互動樣本）、
  `#23` 剩餘項（歷史科目表、官方認列表、歷史學期課程資料；B～F 課程的學分歸屬）、
  `#8`（`Courses.prerequisites` 目前 3,086/3,086 全為 NULL）。
  寫程式解決不了，只能等資料到位。
  （2026-09-18：`#13C`／`#13D` 依專案負責人口頭確認的規則完成，`#20` 隨之完成；`#10`
  已完成現版量測並確認未通過，不再列為外部資料阻塞，已移到上方可立即動工項目。）
...
**#30 已完成，但這輪刻意不接進排課**（見 `#30` 段落與
[變更報告](./2026-09-04-roadmap-30-preference-learning-pipeline.md)）——`Learned_Preference_Weights`
表已經有可讀的權重與 `sufficiency.status`，但 `scheduler.js` 完全沒有被觸碰。**#5B
現在要做的具體工作**：`sufficiency.status === 'sufficient'` 時讀 `easy_weight`（等）
塞進 `preferenceProfile`，`insufficient` 時維持現行的顯式 0/1 邏輯不變；並定義
...
**狀態**：🟡 部分完成（2026-08-31；2026-09-18 更新；2026-09-19 已確認四方案設計）——
**排序機制的根因已修**，詳見
[變更報告](./2026-08-31-roadmap-10-plan-diversity.md)。當時剩下的限制是候選池太小、卡 `#13C`；
`#13C` 已於 2026-09-18 完成，候選池放大，但同日加入的「本系優先」階層讓方案在本系課程上
高度重疊（實測 Persona C 2 種、U1 1 種方案）。剩下的是以目前版本重新量測，不再是外部資料阻塞。

**2026-09-18 興趣輸入補強**：設定頁新增正式修課路徑、依目前候選課 `rag_tag` 動態產生的
主題與自訂關鍵字；資料保存於既有 `User_Profiles.preferences_json`，Agent 在個人化推薦缺少
興趣時也會先詢問方向。這補上先前「興趣 variant 有能力但一般使用者沒有 UI 可以提供真實
興趣」的入口，未改變必修優先或硬限制。興趣設定已完成瀏覽器 A/B。

### 2026-09-18 正式量化驗收

新增 `bench:plan-diversity` 唯讀 runner，以目前 MySQL、正式排課器、三位 demo persona 與
一組無偏好對照重跑。四個 case 的候選池均為 362 門且 hash 相同；門檻為策略保留率至少
75%、有偏好至少 3 個方案、無偏好至少 2 個、中位 Jaccard 不高於 0.75、每對方案至少一門
正式課號不同，以及 hard violation 為 0。

| Case | 策略／方案 | 保留率 | 中位 Jaccard | Hard safety | 結果 |
| --- | ---: | ---: | ---: | :---: | :---: |
| 集中偏好 | 3／1 | 0.3333 | 無比較對象 | 通過 | 失敗 |
| 無偏好對照 | 2／1 | 0.5000 | 無比較對象 | 通過 | 失敗 |
| 挑戰偏好 | 3／1 | 0.3333 | 無比較對象 | 通過 | 失敗 |
| 涼課偏好 | 3／2 | 0.6667 | 0.6667 | 通過 | 失敗（方案數／保留率） |

因此「現版方案數重測」已完成，但結果證明塌縮仍存在；#10 維持部分完成。完整數字見
[2026-09-18-roadmap-10-acceptance.md](./2026-09-18-roadmap-10-acceptance.md) 與
`server/test/reports/plan-diversity-acceptance-latest.json`。

### 2026-09-18 去重前塌縮診斷

同一 runner 現在另存 `plan-diversity-diagnostics-latest.json`。四個 case、每個策略都能對帳為
362 門候選中的 10 門入選與 352 門未入選，而且所有策略最後都達 25 學分上限：

- 集中偏好：綜合、集中加重、較多學分三者的 10 門課完全相同；集中加重改變分數，但前段
  候選仍大量同分，沒有跨過任何入選／落選邊界。
- 無偏好對照：較多學分策略提高學分項後，最終 10 門仍與綜合策略相同。
- 挑戰偏好：加重挑戰改變分數，較多學分也改變前幾門的處理順序，但三者最後仍收斂到同一
  組 10 門課。
- 涼課偏好：涼課加重與綜合策略相同；較多學分策略實際用 `IECS4943`、`ATHL3071` 取代
  `IECS4944`、`IINE1803`，因此保留第二個方案。

未入選原因以 `CREDIT_CEILING` 為最大宗（依 persona／策略為 200～308 門），其次是衝堂、
已修通過、早課限制、無排定時間與同課異班。這表示原始 362 門候選很多，但 25 學分上限和
先前貪婪選擇會把大部分後段候選直接截斷；只放大單一偏好軸未必能讓課程跨過最終入選邊界。
完整欄位與量測見[塌縮診斷報告](./2026-09-18-roadmap-10-collapse-diagnostics.md)。#10 仍維持
部分完成；整張進度表的狀態與相依欄已重新核對，本次沒有下游相依需要改動。

### 2026-09-19 多方案設計決策（使用者確認；任務 1 已實作）

多方案改為四種固定方向，每種有自己的主軸，不再只是把使用者已表達的軸 ×1.5：

| 方案 | 主軸 | 說明 |
| --- | --- | --- |
| 綜合平衡 | 使用者自己的偏好 | 基準方案，最先排 |
| 輕鬆修課 | 涼度 | 使用者表達「喜歡挑戰」時改為「挑戰修課」 |
| 興趣導向 | 興趣 | |
| 集中排課 | 集中度 | |

使用者確認的規則：

1. 使用者表達「喜歡挑戰」時，「輕鬆修課」方案改成「挑戰修課」。
2. 拿掉「較多學分方案」（`personalized_credits`）。
3. 某個方向排不出差異時，合併到既有方案並附原因，不硬湊四種。
4. 任兩個方案之間至少雙向換出、換入各 2 門競爭課（`replacementDistance >= 2`）。

目前設計塌縮的已知原因：

- 使用者沒表達的軸權重為 0，`×1.5` 後仍是 0（`scoringPolicy.js` 的 `resolveScoringPolicy`）。
- `×1.5` 的力道不足以讓課程跨過入選邊界（見上方診斷）。
- 興趣或涼度缺資料時，該軸對所有課程給同分，沒有區分力。

預定參考的文獻與對應做法：

- Hebrard et al. (AAAI 2005)、Trapp & Konrad (IIE Trans. 2015)、Petit & Trapp (IJCAI 2015)：依序求解，每輪加入「與已選方案的最少差異」限制和品質下限。
- Lawler (Management Science 1972)：對最佳方案中的課逐一禁止後重排，取得保證不同的次佳方案。
- Danna & Woodruff (ORL 2009)、MMR (Carbonell & Goldstein, SIGIR 1998)：先產生候選方案池，再挑出彼此差異大的方案。
- Viappiani & Boutilier (NeurIPS 2010)、Nguyen et al. (AIJ 2012)：多方案應代表使用者不同的可能偏好，並以整組方案的期望效用評估。

演算法由使用者參考上述論文後修改；驗收沿用 `bench:plan-diversity` 的既有門檻。

### 2026-09-19 求解器 spike（任務 1 第一步）

文獻落地拆成四個任務：
1. 多方案核心，產生候選池（Petit & Trapp 2015 為主，Hebrard 2005、Trapp & Konrad 2015 補充）
2. 從候選池挑出最後的方案（Danna & Woodruff 2009）
3. 個人化（Viappiani/Faltings/Pu 2006、Dragone 2018）
4. 系外與通識探索（Pardos & Jiang 2020）

任務 1 的求解器決策：
- 替代方案改用 HiGHS（MILP）加上 Dinkelbach。
- 綜合平衡方案 S₀ 保留 greedy。

用真實候選池測試基本模型，結果是 GO：
- 四個 case 都回傳 optimal。
- validator 0 violation。
- warm p95 ≤ 34 ms。
- 重跑結果一致。
...
兩件事被改回舊行為，這次修正：評價數下限回到 `max(2, S₀ 有評價課數)`（等於要求替代方案的
評價覆蓋率不得低於 S₀，主軸還沒比較就先無解），以及主軸門檻的可達範圍檢查被移除。少了可達
範圍檢查，S₀ 已經是候選池最涼／最合興趣的組合時，系統會把「資料上做不到」說成
`axis-threshold-infeasible`——那是兩回事。

現在：評價數下限為 `max(2, ⌈S₀ 有評價課數 ÷ 2⌉)`；興趣、輕鬆、挑戰都以實際門檻
`baseline ± minGain` 對「競爭課 ∪ 固定課」的極值檢查可達範圍（固定課的分數也算進平均，
不納入會誤判）；做不到記 `no-signal` 並附 `detail`，做得到卻湊不出合法方案才由求解後的診斷
分類。5 case 真實資料驗收從 3 通過變成 4 通過，未通過的是挑戰 persona 的集中主軸——
`resolvedBy` 為 axis／quality／hierarchy，是三條限制同時成立才無解，不是訊號問題。
87% 品質、階層門數相等、雙向換課 2 門、學分不少於 S₀ 與 `minGain = 0.02` 都沒有放寬。
瀏覽器 A/B（真實帳號 D1249697）：方案數由 1 變成 3，興趣與集中兩軸不再被誤報成不可行，
合併的輕鬆主軸顯示「綜合方案已達目前課程資料可改善的界線」，切換方案後課程集合確實不同，
console 無新增錯誤。求解預算也恢復為線上 K=1／2.5 秒，benchmark 另記 K=3。
詳見[訊號判定修正報告](./2026-09-20-roadmap-10-axis-signal-correction.md)。


### 2026-09-19 任務 1 正式實作結果

替代方案已改由 HiGHS 解 Dinkelbach 的 MILP 子問題；S₀ 仍由原 greedy 產生。模型把學分
對齊、每對方案雙向換課至少 2 門、主軸改善、固定課與課程規則寫成限制。原本的
`−2500`／`−5000` 階層懲罰不再放進 87% 品質分母，改成跨年級與系外競爭課門數各自與 S₀
相同，讓品質門檻衡量同階層內的偏好取捨，同時不破壞本系優先。

`minGain` 以真實資料由 0.05 校準為 0.02。最新 5 case 結果為 3 通過、2 未通過：未通過者都沒有
違反品質、學分或安全限制，而是可用訊號或主軸可行性不足。#10 因任務 2～4 與這兩個案例仍未完成，
維持部分完成。完整內容見[任務 1 變更報告](./2026-09-19-roadmap-10-milp-candidate-generation.md)。整張進度表的
狀態與相依已重新核對；本次沒有其他任務的狀態或相依需要改動。

> 2026-09-20 補正：上面「0.04 仍讓無偏好涼度主軸不可行」的說法，是在主軸可達範圍檢查被移除的
> 狀態下量到的，已不再成立，故刪除。修正訊號判定後重跑：0.02 與 0.05 都是 5 case 中 4 個通過，
> 但 0.05 會讓 `no-preference-control` 與 `persona-easy` 的涼度主軸落在可達範圍外（`no-signal`／
> `threshold-unreachable`），實際方案數由 3 降為 2——驗收仍算通過，因為 no-signal 不計入分母。
> 保留 0.02 是為了多給一個真的能用的方案，不是為了讓驗收好看。詳見
> [訊號判定修正報告](./2026-09-20-roadmap-10-axis-signal-correction.md)。
...
**狀態**：✅ 已完成（2026-09-18）——見[變更報告](./2026-09-18-implement-13c-13d-eligibility-rules.md)

**相依**：#13B（已完成）

**規則來源**：專案負責人 2026-09-18 口頭逐項回答（記錄於 `docs/DEPARTMENT_MAPPING.md`），
**不是**系辦或課務組的書面規則。下方驗收標準第一條「每一條規則都能指向書面來源」因此以
「本人確認，2026-09-18」作為來源（本人同意這個作法）；日後若取得校方文件，需回頭核對
`classKindCatalog.js` 的 `ELIGIBILITY_RULES`。

**各項結論**：13C-1、13C-2 依年級判定；13C-3 依 `COLLEGE_DEPARTMENTS` 判定學院；13C-4
（他系選修）不處理，系統只服務資訊工程學系；13C-5（同系他年級選修）可修，但排序在本年級
選修之後（`CROSS_YEAR_ELECTIVE_PENALTY`）。實作時另依本人決定加上「本系優先」排序階層與
「同一系列 (一)(二) 不排同學期」，理由見變更報告。

**原本的開始條件**（保留作為歷史）：#13B 已把不確定性顯性化；取得下表各項的正式書面規則。在取得前不得以臆測填入判定邏輯。
...
**狀態**：✅ 已完成（2026-09-18）——本人確認：13D-1 D 類是獨立學制，不在
`User_Profiles.department` 的值域內，一律不可修；13D-2 學分學程不需報名，因此不需要
「已報名學程」欄位；13D-3 F 類排除於系統外；13D-4 `department` 值域為 A 表系所全名或
學士綜合班名稱（不含社會創新、創能、碩士綜合班），綜合班學生大三起改成分流後系所。
下方驗收標準第一條（Profile 擴充已報名學分學程）因規則改變而不再需要。

**原本的狀態**：🟡 工程可開始——正式驗收等待特殊身分資料
...
**狀態**：✅ 已完成（2026-09-18）——active term 過濾、`eligibilitySource`／`scopeReason`／`term` 三欄位、四種判定的正式對照於 2026-08-15 完成；剩下的 B～F 可加選規則隨 #13C／#13D 於 2026-09-18 完成（原本的盤點狀態：🟡 部分完成）

**相依**：#12、#13A、#13B、#13C、#13D、#18、#19（**均已完成**）
...
**狀態**：🟡 部分完成（2026-09-21 更新）——版本化架構、逐門認列追溯、規則來源與
待確認狀態、補學分推薦驗證與候選範圍、通識基礎必修／選修拆分與依入學年度選規則均已完成；
正式排課也已使用「畢業缺口 ÷ 剩餘學期」分配本學期的本系選修、通識與系外課程；
...
**2026-09-21 接入單學期排課**：新增共用 `graduationPlanning`，由同一份版本化規則與
`User_Course_History` 計算缺口，剩餘學期可由 Profile 明確設定，否則依年級與 active term
推算。正式順序為「當期必修／必修重補修 → 最多 3 門且不超過當期目標的本系選修 →
依通識／系外當期需求選 2 門廣度課」。類別配額完成後不再為填滿 25 學分加入更多選修；
候選不足時保留缺口。HiGHS 替代方案以硬限制維持 S₀ 的三類門數，避免多方案破壞畢業配置。
詳見[變更報告](./2026-09-21-graduation-aware-semester-allocation.md)。
...
**相依**：#26 已完成；#10 已提供本任務需要的多方案介面與塌縮資訊，但完整多樣性驗收仍為
部分完成，不影響 #27 已交付的比較 UI 與 counterfactual 功能。
...
**狀態**：✅ **已完成（2026-09-04）**——見[變更報告](./2026-09-04-roadmap-30-preference-learning-pipeline.md)。
四條驗收標準全數達成；**這輪刻意不接進排課**，`scoreCourse()`／`buildPreferenceProfile()`
維持現狀不變，見下方「2026-09-04 完成內容」。
...
實測 demo 帳號（真實 MySQL）：真正的使用者行為事件只有 34 筆、來自 1 個人、
而且是開發測試期間累積的——那個量級拿去學就是把雜訊當成個人化。因此這輪的
驗收重心放在「機制正確」而非「demo 帳號真的學到東西」：

- 新增 `server/src/skills/preferenceLearning.js`（純函式）：把 `course_withdrawn`
  的退課原因、`recommendation_accepted`（要求曝光時有兩個以上方案可選才算數）、
  `course_viewed`（固定映射到 `interest`，累計貢獻鎖在單一強訊號以下，看了又退
  掉的不計）折成 `{ interest, compact, easy }` 三軸權重，重用 `reviewStats.js`
  既有的 m-estimate 往顯式基準收縮，顯式設定用 `Math.max` 頂住下限。
- **資料量門檻**：`REQUIRED_USABLE_EVENT_COUNT = 50`，低於門檻時
  `sufficiency.status` 為 `insufficient`，回傳權重等於顯式設定原樣，不是半調子
  的學習值。Demo 帳號實測落在 `31/50`——如實記錄，不調低門檻讓數字好看。
- 新表 `Learned_Preference_Weights`（migration `006`），鍵為 HMAC `subject_id`，
  已接上 `#33` 既有的刪除與匯出路徑（`privacyPolicy.js` 早就承諾的
...
**2026-09-04 更新：`#30` 完成後的現況**

- `Learned_Preference_Weights` 表已存在，`preferenceLearningService.js` 提供
  `deleteLearnedWeights()`（單一 subject 清除）與 `cleanupExpiredLearnedWeights()`
  （保存期限到期清除），兩者都已接上 `#33` 的刪除路徑與 `npm run cleanup:privacy`。
  **`#31` 要做的重設功能，資料層已經備妥，剩下的是 API／UI**（例如
  `POST /api/profile/reset-learned-weights` 這類使用者可自行觸發的入口，目前只有
  帳號整個刪除會連帶清掉，沒有「只重設學習權重、保留顯式 Profile」的單獨功能）。
- `sufficiency.status` 三態（`sufficient`／`insufficient`／`no-consent`）已經是
  「使用者能看見目前用的是顯式還是學習」的資料基礎——`insufficient` 時權重等於
  顯式設定原樣，這件事本身就是可以直接顯示給使用者的來源標示；**目前只是還沒接
  到任何 UI**。
- **仍未完成**：「少量互動」與「長期未使用」的細分門檻（`#30` 只有一個總量門檻，
  沒有按軸或按時間窗切分）；時間衰減（`courseHistory` 已含學年度與學期欄位，
