...
| 10 | 修復多方案塌縮：5 個 variant 實際只產出 2 種課表 | 🟡 部分完成（2026-09-18 更新）——必修安全邊界、涼度來源與興趣接線已修；#7 已用動態策略取代固定 variant。**候選池限制已隨 #13C 解除**（demo 帳號可競爭課程 16 → 365 門），但同日加入的「本系優先」階層讓各方案在本系課程上高度重疊，2026-09-18 實測 Persona C 2 種、U1 1 種方案；**需以目前版本重新量測**（推甄資料包 P0-5），不再是外部資料阻塞 | #4、#21、#22、#7、#13C（均已完成） |
