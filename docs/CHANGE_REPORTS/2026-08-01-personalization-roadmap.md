...
| 5B | per-user 加權方向（同一分數對不同使用者相反符號） | ✅ **已完成（2026-09-05）**——見下方 #5B 段落與[變更報告](./2026-09-05-roadmap-5b-per-user-preference-direction.md)。方向來自顯式的 `#涼課優先`／`#挑戰難課` 標籤（事件 schema 表達不出「太簡單」，方向不從行為推論，見 `docs/DECISIONS.md` ADR-022），強度由學到的權重加強；只接進方案層的 `evaluatePreference()`，單一門課排序（`scoreCourse()`）留給 #7 | #5A、#29、#33、#2、#30（均已完成） |
...
| 30 | 建立可重現的 per-user preference update pipeline | ✅ **已完成（2026-09-04）**——見下方 #30 段落與[變更報告](./2026-09-04-roadmap-30-preference-learning-pipeline.md)。四條驗收標準全數達成；**這輪刻意不接進排課**，接進方案層由 `#5B`（2026-09-05）完成 | #2、#5A、#29（均已完成） |
| 31 | 建立冷啟動、偏好重設、時間衰減與資料不足策略 | ✅ **已完成（2026-09-04）**——見下方 #31 段落與[變更報告](./2026-09-04-roadmap-31-cold-start-reset-decay-and-source.md)。四條驗收標準全數達成；時間衰減在今天的真實資料上是數學上的 no-op（已明講）；排課引擎已於 `#5B`（2026-09-05）接進方案層讀取學到的權重，`appliedToScheduling` 旗標據實回報 | #18、#30（均已完成） |
| 32 | 比較 content-based、collaborative filtering 與 hybrid 方法 | ⬜ 未開始（2026-09-04：#31 已完成，不再是阻塞；仍卡 #6、#7、#36 與樣本量） | #6、#7、#31（已完成）、#36；另需足夠互動樣本 |
...
| 36 | 建立 personalization baseline 與 preference sensitivity A/B | 🟡 部分完成——**#30、#31、#5B 均已完成，不再是阻塞**；仍卡 #7（連續權重向量取代五個固定 variant）的剩餘工作 | #5B、#7、#30、#31（#7 除外均已完成） |
...
| ~~31~~ | ~~冷啟動、偏好重設、時間衰減與資料不足策略~~ | **已於 2026-09-04 完成**（見下方 #31 段落）。它是 `#36`／`#32` 剩餘的相依之一，解除後 `#36` 只剩 `#5B`／`#7`、`#32` 只剩樣本量 |
| ~~5B~~ | ~~per-user 加權方向~~ | **已於 2026-09-05 完成**（見下方 #5B 段落）。`#36` 因此只剩 `#7` 這一個相依 |
...
- `#30` 那條鏈（`#5B`／`#7`／`#31`／`#36`／`#9`／`#6`／`#32`）**2026-09-04 起不再卡在
  單一閘門**，各自剩下的是自己的實作工作量，見各任務段落。`#31` 與 `#5B` 已分別於
  2026-09-04、2026-09-05 完成，`#36`、`#32`、`#6` 因此各自少了相依，只剩 `#7` 與
  樣本量。
...
| Gate 3 — 偏好資料與學習 | 先合法、安全記錄互動，再更新個人權重 | #29、#33、#2、#30、#31（均已完成）、#7 | 協同過濾、探索與 hybrid 比較 |
...
**狀態**：⬜ **可開始（2026-09-04：#30 已完成）**——`#5B`（2026-09-05）已經在
方案層示範了「顯式方向 + 學到的強度」這個模式（見該段落與
[變更報告](./2026-09-05-roadmap-5b-per-user-preference-direction.md)），
`#7` 把同一套模式接進單一門課排序時可以直接沿用，不需要重新設計。
...
**狀態**：⬜ 未開始（2026-09-04：#31 已完成，不再是阻塞；仍卡 #6、#7、#36 與樣本量）

**相依**：#6、#7、#31（已完成）、#36；外部相依為達到事前定義最低門檻的去識別化互動樣本。
...
**狀態**：🟡 部分完成——已有興趣偏好與 avoid-time 的最小敏感度 A/B；**`#30`、`#31`、
`#5B` 均已完成，不再是阻塞**；完整 baseline、量化指標與行為差異仍卡 `#7`
（連續權重向量取代五個固定 variant，需要接進單一門課排序才有東西可比較）
