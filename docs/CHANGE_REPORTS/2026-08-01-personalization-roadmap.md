...
| 13D | 學制、學程與特殊身分 | 🟡 **資料欄位已接線，正式規則仍等待外部資料**（2026-09-11）——`program_type`／`enrolled_programs`／`college` 已可經 Profile API 讀寫，研究所課程以 `gradeLevel=5` 處理；B～F 類與特殊身分的正式適用結論仍不推定 | #13B、#18（均已完成）；剩餘外部條件為特殊身分資料與正式適用規則 |
