...
| 23 | 建立版本化且可追溯的畢業規則引擎 | 🟡 部分完成（2026-08-31 兩輪）——版本化架構、逐門認列追溯、規則來源記錄、補學分推薦驗證與候選範圍、通識基礎／選修拆分皆完成；剩餘各項全部卡在外部資料（科目表、官方認列表、歷史學期資料、系辦確認）。**2026-09-18 新增缺口**：#13C 讓學院綜合班、學分學程、外語選修等 B～F 課程可以修，但它們算哪一類畢業學分沒有規則——畢業頁補學分推薦對 B～F 仍只推通識，排課的 `graduationCredits` 則暫時把它們計入 | #12、#19；另需校方正式規則 |
