...
| 10 | 修復多方案塌縮：5 個 variant 實際只產出 2 種課表 | 🟡 **部分完成（2026-09-18 驗收未通過）**——已建立真實 MySQL 唯讀 benchmark、正式門檻與去重前診斷；候選池均為 362 門，但集中／挑戰 persona 皆為 1/3、涼課 persona 為 2/3、無偏好對照為 1/2，未達 75% 保留率。診斷確認所有策略均達 25 學分，集中／挑戰加權只改分數或處理順序、沒有改變最終課程集合；涼課只有較多學分策略替換 2 門課。見下方、[驗收報告](./2026-09-18-roadmap-10-acceptance.md)與[塌縮診斷](./2026-09-18-roadmap-10-collapse-diagnostics.md)。**2026-09-19**：已確認四方案規則並把文獻落地拆成四個任務；任務 1 的 HiGHS 求解器 spike 為 GO（4 case 全 optimal、validator 0 violation、warm p95 ≤ 34 ms），見 [spike 報告](./2026-09-19-roadmap-10-highs-spike.md)。尚缺：正式 MILP 計畫與實作（Dinkelbach、主軸門檻、「在上限內排滿」語意、候選池）、任務 2～4 | #4、#21、#22、#7、#13C（均已完成）；任務 2 另需 Danna & Woodruff 2009 全文（外部，付費論文，待使用者提供） |
...
## 現在可以動工的任務（2026-09-18 第七次盤點，依建議順序排列）

`#10` 已於 2026-09-18 完成真實 MySQL 量測，但四個 case 全部未達完整驗收門檻；這不是
外部資料阻塞，而是已由可重跑報告定位的工程缺口，因此重新列入可立即動工項目。
...
| 順位 | # | 任務 | 為什麼排在這裡 |
| ---: | ---: | --- | --- |
| 1 | 10 | 修復多方案塌縮 | 前置全數完成，真實 benchmark 已可重播且明確失敗：集中／挑戰 persona 只剩 1/3，涼課 2/3，無偏好 1/2；2026-09-19 已完成 HiGHS spike（GO），下一步是寫正式 MILP 計畫並實作任務 1，不需等待外部資料（任務 2 才需要 Danna & Woodruff 全文） |
...
4. 任兩個方案之間至少差 2 門非必修課（Hamming 距離 d = 2）。
...
同樣學分下，MILP 的靜態分數比 greedy 高約 4,000。另外發現 greedy 的分數只負責排順序，不代表值不值得選，所以正式模型要把「在上限內排滿」寫成明確的語意。詳見[spike 報告](./2026-09-19-roadmap-10-highs-spike.md)。正式的 MILP 設計計畫尚待撰寫，#10 仍為部分完成。
