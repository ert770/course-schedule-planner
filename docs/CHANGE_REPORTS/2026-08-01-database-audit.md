...
**嚴重度**：🟠 高　**狀態**：✅ 已完成（2026-08-02）——詳見 [修復 department 含字面單引號](./2026-08-02-department-quote-d3.md)

實際儲存值為 `'資訊工程學系'`——**包含單引號字元本身**，不是 SQL 引號（HEX 前後皆為 `27`）。

任何字串比對（包含 `#13` 要做的系所對照）都會失敗，除非先去除引號。

**「是匯入缺陷還是全欄位問題」已釐清**：2026-08-02 掃描 `defaultdb` 全部 19 個文字欄位，只有 `User_Profiles.department` 前後帶引號（1/1 筆），`Courses.dept`、`name`、`teacher`、`room`、`Reviews_tags` 等其餘 18 個欄位皆為 0。**屬單一欄位的匯入缺陷，非全表通例**，因此課程端不需要做同樣處理。

修復方式為 `server/src/utils/text.js` 的 `normalizeDepartment()`，於讀取與寫入兩端皆套用，並在第一次遇到髒值時寫入警告日誌；資料庫中那 1 筆資料亦經使用者確認後清理。
...
| ✅ | D3 `department` 引號 | 已修復：讀寫兩端正規化，資料庫該筆亦已清理，`#13` 的系所比對前提解除 |
| 1 | D7、D6 | 顯示層與資料一致性 |
| 2 | D4、D5 | 需要外部資料來源，非程式可解 |
